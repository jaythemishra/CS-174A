...
{ constructor( context, control_box )                   // a Scene that is a physics demo.  This technique is careful to totally
    { super(   context, control_box );                  // decouple the simulation from the frame rate.
      Object.assign( this, { time_accumulator: 0, time_scale: 1, t: 0, dt: 1/20, bodies: [], aliens: [], steps_taken: 0 } );
    }
  simulate( frame_time )                              // Carefully advance time according to Glenn Fiedler's "Fix Your Timestep" blog post.
    { frame_time = this.time_scale * frame_time;                   // This line lets us create the illusion to the simulator that
      ...
      { this.update_state( this.dt );                              // Single step of the simulation for all bodies.
        ...
        for( let b of this.suns ) b.advance( this.dt );
        for( let b of this.moons ) b.advance( this.dt );
        for( let b of this.aliens ) {
          b.body.advance( this.dt );
          b.ring.advance( this.dt );
        }
      ...
      for( let b of this.aliens ) {
        b.body.blend_state( alpha );
        b.ring.blend_state( alpha );
      ...
      for( let b of this.suns ) b.blend_state( alpha );
      for( let b of this.moons ) b.blend_state( alpha ); 
  ...
  display( graphics_state )
    { if( !graphics_state.lights.length ) graphics_state.lights = [ new Light( Vec.of( 7,15,20,0 ), Color.of( 1,1,1,1 ), 100000 ) ];
      ...
      for( let b of this.suns )
        b.shape.draw( graphics_state, b.drawn_location, b.material );   // Draw each shape at its current location.
      for( let b of this.moons )
        b.shape.draw( graphics_state, b.drawn_location, b.material );   // Draw each shape at its current location.
      for( let b of this.aliens ) {
          b.body.shape.draw( graphics_state, b.body.drawn_location.times( Mat4.scale([1,1,1])), b.body.material );
          b.ring.shape.draw( graphics_state, b.ring.drawn_location.times( Mat4.scale([0.8,0.8,0.1])), b.ring.material );
      }
...
// BODY for Collision Detection BODY for Collision Detection BODY for Collision Detection BODY for Collision Detection BODY for Collision Detection
// BODY for Collision Detection BODY for Collision Detection BODY for Collision Detection BODY for Collision Detection BODY for Collision Detection
